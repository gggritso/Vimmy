(function() {
  ...
  var Vimmy = {
    ...
    'scrollDistance': 175,  // pixel distance for hjkl scrolling
    ...
    'isBlacklisted': false,
    'hostBlacklist': [ 'www.facebook.com', 'twitter.com', 'www.tumblr.com', 'getprismatic.com',
      'app.asana.com', 'www.feedly.com', 'www.buzzfeed.com' ],

    'hintTemplate': function( hint ) {

      return '<span class="vimmy-hint"><b>' +  hint.split('').join('</b><b>') + '</b></span>';
    },
    ...
    'followLink': function( $link ) {
      ...
      if ( destination === 'same' ) {
        ...
        chrome.runtime.sendMessage({ message: 'newTab', url: href });
      }

      Vimmy.typedCharacters = [];
      $( '#vimmy-hints span' ).show();
    ...
    'showHints': function( ) {
      ...
      var links = $( 'a:visible:in-viewport' ),
      ...
      $( '#vimmy-hints' ).append( hints ); // append everything in one shot

    },

    'hideHints': function() {
      $( '#vimmy-hints' ).remove(); // since they're generated for only chunks of pages, it's easier
    ...
    'checkHint': function() {
      ...
      $( '#vimmy-hints b' ).removeClass( 'typed' );
      $( '#vimmy-hints span' ).show();
